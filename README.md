...
:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for Wave Financial (beta)](https://www.cdata.com/download/download.aspx?sku=HWZK-V&type=beta). 
## Purpose
We created this read-only MCP Server to allow LLMs (like Claude Desktop) to query live data Wave Financial supported by the [CData JDBC Driver for Wave Financial](https://www.cdata.com/drivers/wavefinancial/jdbc).
...
1. Clone the repository:
      ```bash
      git clone https://github.com/cdatasoftware/wave-financial-mcp-server-by-cdata.git
      cd wave-financial-mcp-server-by-cdata
      ```
2. Build the server:
      ```bash
      mvn clean install
...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/wavefinancial/download/jdbc](https://www.cdata.com/drivers/wavefinancial/download/jdbc)
3. License the CData JDBC Driver:
    * Navigate to the `lib` folder in the installation directory, typically:
        * (Windows) `C:\Program Files\CData\CData JDBC Driver for Wave Financial\`
        * (Mac/Linux) `/Applications/CData JDBC Driver for Wave Financial/`
    * Run the command `java -jar cdata.jdbc.wavefinancial.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.wavefinancial.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `wave-financial.prp`) using the following properties and format:
    ...
    * **Tables** - leave blank to access all data, otherwise you can explicitly declare the tables you wish to create access for
      ```env
      Prefix=wavefinancial
      ServerName=CDataWaveFinancial
      ServerVersion=1.0
      DriverPath=PATH\TO\cdata.jdbc.wavefinancial.jar
      DriverClass=cdata.jdbc.wavefinancial.WaveFinancialDriver
      JdbcUrl=jdbc:wavefinancial:InitiateOAuth=GETANDREFRESH;
      Tables=
      ```
